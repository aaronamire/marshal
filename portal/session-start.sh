...
export XDG_SESSION_DESKTOP=leaves
...
# Respects OnlyShowIn/NotShowIn for the "leaves" desktop.
launch_autostart_entry() {
    ...
    # Check OnlyShowIn (if present, must include "leaves")
    only=$(grep -i '^OnlyShowIn=' "$desktop_file" 2>/dev/null | cut -d= -f2)
    if [ -n "$only" ]; then
        echo "$only" | tr ';' '\n' | grep -qi 'leaves' || return
    fi

    # Check NotShowIn (if present, must not include "leaves")
